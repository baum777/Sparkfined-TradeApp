/**
 * Jupiter API response type guards (backend)
 * Mirrors shared/trading/fee/feeQuote.ts JupiterQuoteResponseLike.
 */
...
export function isJupiterQuoteResponseLike(value: unknown): value is JupiterQuoteResponseLike {
  return isRecord(value) && typeof (value as Record<string, unknown>).outAmount === 'string';
}
