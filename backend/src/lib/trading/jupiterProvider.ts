 ...
 * Canonical backend port of api/_lib/trading/swap/jupiterProvider
 */

import { getConfig } from '../../config/config.js';
import { badRequest, internalError } from '../../http/error.js';
import type { SwapProvider, GetQuoteParams, QuoteResult, GetSwapTxParams, SwapTxResult } from './types.js';
import { isJupiterQuoteResponseLike, parseJupiterSwapResponse } from './jupiterTypes.js';
...
function assertPlatformFeeConfigured(feeBps: number) {
  if (feeBps <= 0) return;
  const { JUPITER_PLATFORM_FEE_ACCOUNT } = getConfig().env;
...
export const jupiterProvider: SwapProvider = {
  ...
  async getQuote(params: GetQuoteParams): Promise<QuoteResult> {
    ...
    const baseUrl = getConfig().env.JUPITER_BASE_URL.replace(/\/$/, '');
