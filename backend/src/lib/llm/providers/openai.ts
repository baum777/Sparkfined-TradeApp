...
export async function callOpenAI(options: ProviderCallOptions): Promise<ProviderResult> {
  ...
  if (!env.OPENAI_API_KEY) {
    const err = new FetchJsonError('OpenAI API key missing');
  ...
  const result = await withRetry(
    async () => {
      const res = await fetchJson<OpenAIChatCompletionResponse>(url, {
        method: 'POST',
      ...
      if (!res.ok) {
        const err = new FetchJsonError(`OpenAI upstream error (${res.status})`, {
          status: res.status,
          bodyText: res.text,
      ...
      if (typeof text !== 'string' || text.trim().length === 0) {
        const err = new FetchJsonError('OpenAI response missing message.content', {
          status: res.status,
          bodyText: res.text,
          responseHeaders: res.headers,
        });
