...
export function useSessionReviewInsight(input: {
  ...
  const queryKey = useMemo(
    () => ['reasoning', 'session-review', input.referenceId, version, contextKey] as const,
  ...
  const query = useQuery({
    queryKey,
    queryFn: () => reasoningApi.sessionReview({ referenceId: input.referenceId, context, version }),
